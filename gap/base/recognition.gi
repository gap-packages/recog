...
InstallMethod( IsCorrect, [ IsRecogNode ],
  function(ri)
    local h;
    # If verification was already done (possibly by other means than constructing
    # a presentation)
    if HasIsCorrect(ri) then
        return IsCorrect(ri);
    fi;
    # Check that all generators of Grp(ri) lie in the group generated by NiceGens(ri).
    # This check is also performed in RecogniseGeneric, but it should be cheap,
    # so let's be safe.
    for h in GeneratorsOfGroup( Grp(ri) ) do
  ...
  function(H, methoddb, depthString, knowledge)
    ...
    if IsLeaf(ri) then
        ...
        if not HasStdPresentation(ri) and not HasCalcStdPresentation(ri) then
            SetCalcStdPresentation(ri, CalcStdPresentationGenericLeaf);
        fi;
        if InfoLevel(InfoRecog) = 1 and depth = 0 then Print("\n"); fi;
        # StopStoringRandEls(ri);
        SetIsCorrect(ri,true); # Leaves are always correct
        SetFilterObj(ri,IsReady);
        return ri;
    fi;
...
InstallGlobalFunction( CalcStdPresentationGenericLeaf,
  # generic (and bad!) method for a leaf node
  function(ri)
    ...
    if HasSize(ri) and Size(ri) = 1 then
        ...
        # (One might expect that NiceGens(ri) has only one element
...
InstallGlobalFunction( CalcStdPresentationGenericNonLeaf,
  # generic method for a non-leaf node
  function(ri)
    ...
    # induces an isomorphism between the central quotients) and hence there is
    # nothing to do: We just take the presentation from riFac.
    if riKer = fail then
    ...
    # Steps B and C may fail in rewriting certain elements as SLPs.
    # In this case, we abort in the following way.
    presAbort := function()
        Info(
            InfoRecog, 1,
    ...
    # We do not set IsCorrect(ri) to true because a full verification also requires the
