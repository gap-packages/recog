...
function(ri,G)
  ...
  if res = fail then
      ...
      if Length(res.orb) * Length(res.orb[1]) <> ri!.dimension then
          ...
          InitDataForKernelNode(ri).t := Concatenation(res.orb);
          InitDataForKernelNode(ri).blocksize := Length(res.orb[1]);
          AddMethod(InitDataForKernelNode(ri).hints, FindHomMethodsProjective.DoBaseChangeForBlocks, 2000);
...
function(ri,G)
  ...
  InitDataForImageNode(ri).blocksize := ri!.blocksize;
  AddMethod(InitDataForImageNode(ri).hints, FindHomMethodsProjective.Blocks, 2000);
...
function(ri,G)
  ...
  od;
  # For the image:
  InitDataForImageNode(ri).blocks := blocks;
  ...
  AddMethod(InitDataForKernelNode(ri).hints, FindHomMethodsProjective.BlocksBackToMats, 2000);
  return Success;
end);
...
function(ri,G)
  ...
  InitDataForImageNode(ri).blocks := ri!.blocks{[1..Length(ri!.blocks)-1]};
...
#  InitDataForImageNode(ri).blocksize := ri!.blocksize;
#  AddMethod(InitDataForImageNode(ri).hints, FindHomMethodsProjective.BalTreeForBlocks, 2000);
#
#  # Inform authorities that the kernel can be recognised easily:
#  InitDataForKernelNode(ri).subdim := subdim;
#  InitDataForKernelNode(ri).blocksize := ri!.blocksize;
#  AddMethod(InitDataForKernelNode(ri).hints, FindHomMethodsProjective.BalTreeForBlocksProjKernel, 2000);
#
