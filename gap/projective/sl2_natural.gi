...
##
## Strategy:
##   1. Find an element xm of order 4 (whose eigenvalues are Z(5) and 1/Z(5)).
##      The base change matrix mat is built from the eigenvectors of xm.
##   2. Find fitting transvections via random walk.
RECOG.ConRecogNaturalSL25 := function(G,f)
  ...
  repeat
    ...
    od1 := x[1,2];
    od2 := x[2,1];
  until (( (d1 = Z(5) and d2 = Z(5)^4) or (d1 = Z(5)^4 and d2 = Z(5)) )
          and (od1 = zero or od2 = zero))
     or ( (d1 = one and d2 = Z(5)^2) or (d1 = Z(5)^2 and d2 = one) )
     or ( d1 = zero and d2 = zero );

  # Build base change matrix mat from eigenvectors of xm.
  # Columns of mat are eigenvectors for 1/Z(5) and Z(5) respectively,
  # so that xm^mat = diag(1/Z(5), Z(5)).
  a := 1/Z(5);
  mat := [[-x[1,2], -x[1,2]], [x[1,1]-a, x[1,1]-1/a]];
  if mat[1,1] = zero and mat[2,1] = zero then
    mat[1,1] := x[2,2]-a;   mat[2,1] := -x[2,1];
  ...
  # diagonal entries Z(q)^0, i.e. diagonal entries are not Z(5)^2, 
  # which has order 5
  trupm := fail;
  trlowm := fail;
  cur := One(GM);
  while trupm = fail or trlowm = fail do
    ...
    if smm[2,1] = zero and smm[1,2] <> zero and trupm = fail then
      if smm[1,1] = smm[2,2] and smm[1,1] <> Z(5)^2 then
        trupm := cur;
      fi;
