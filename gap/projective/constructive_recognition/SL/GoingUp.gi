...
local DoColOp_n,DoRowOp_n,FixSLn,Fixc,MB,Vn,Vnc,aimdim,c,c1,c1f,cf,cfi,
    ...
    # We already have an SLP for an n-1-cycle: it is one of the std gens.   # <-- NOTE: this text matches the current paper, but not the code
    ...
    if w.n > 2 then
        if IsOddInt(w.n) then
            ...
            else   # in even characteristic we take the n-cycle:   # <-- NOTE: what happens here if we don't???
    ...
    while true do   # will be left by break
        ...
        repeat
            ...
            w.count := w.count + 1;
            c1 := PseudoRandom(w.sld);  # TODO: can't we do better by using a conjugate of s??
