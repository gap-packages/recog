...
ClassicalForms_InvariantFormDual := function( module, dmodule )
    ...
    Info( InfoClassical, 2, "found homomorphism between V and V^*" );
    ...
    if TransposedMat(form) = -form  then
        Info(InfoClassical, 2, "form is symplectic" );
        if Characteristic(field) = 2  then
        ...
        Info(InfoClassical, 2, "form is symmetric" );
        ...
        Info( InfoClassical, 2,"unknown form" );
...
ClassicalForms_InvariantFormFrobenius := function( module, fmodule )
    ...
    Info( InfoClassical, 2,"found homomorphism between V and (V^*)^frob" );
...
InstallGlobalFunction(ClassicalForms, function( arg )
    ...
    if not MTX.IsAbsolutelyIrreducible(module)  then	
        Info( InfoClassical, 2,  "grp not absolutely irreducible" );
