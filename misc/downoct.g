          ...
          if IsSubset(Set([1,d-1,dim-d]),Set(dims)) then
             ...
             repeat 
                ...
                if comm1<>One(a) and comm2<>One(a) and 
                  ...
                  Add(action, [Coefficients(basis,basis[1]*a),
                               Coefficients(basis,basis[2]*a)]);
                  Add(action, [Coefficients(basis,basis[1]*b),
                               Coefficients(basis,basis[2]*b)]);
                  Add(action, [Coefficients(basis,basis[1]*c),
                               Coefficients(basis,basis[2]*c)]);
                  image:=Group(action);
                  count:=0;
                  readyqpl1:=false;
                  readyqm1:=false;
