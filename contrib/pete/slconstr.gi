...
local  t, tinv, B, H, m, x, y, q, count, new, tau, i, j;     
     ...
     while count<m do
       ...
       if not x=tinv*x*t then
          count:=count+1;
       # was: elif IsInTranGroup(bbg,H,x) then
       elif x in H then
          count:=count+1;
       else Add(B,x);
...
#########################################################################
##
#F SLSLPbb ( <data structure for bbg>, <bbg element or matrix>, 
#                 <dimension> )
##
   ...
   repeat
      ...
      if not One(bbg)= cand  and CommutesWith( bbg , S , cand ) then
         if Length(S) = 1 then
            ...
            t1new:=Comm(u1 , cand );
            # was: if not IsInTranGroup( bbg , T1 , t1new ) then
...
return fail;
   ...
   repeat
     ...
     # was: if not IsInTranGroup(bbg, trangp, comm) then
...
return fail;
   ...
   # was: if not ForAll(Q, x->IsInTranGroup( bbg, trangp, Comm(x,t31) ) ) then
