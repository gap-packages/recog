# $Id: memory.gi,v 1.2 2005/03/04 23:08:23 gap Exp $
...
InstallGlobalFunction( SLPOfElm,
  function(elm)
    ...
    # generators.
    if elm!.n = 0 then
        return StraightLineProgram( [[1,0]], elm!.slp.nogens );
    else
...
InstallMethod( Length, "for a matrix with memory",true,
  [ IsMatrix and IsObjWithMemory ], 0,
  function(M)
    return Length(M!.el);
  end);
