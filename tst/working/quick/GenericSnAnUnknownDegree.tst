...
>     RECOG.TestGroup(altPermGroups[i], false, Factorial(dataPerm[i, 1])/2);
>     RECOG.TestGroup(symPermGroups[i], false, Factorial(dataPerm[i, 1]));
> od;
gap> for i in [1 .. Length(dataMat)] do
>     RECOG.TestGroup(altMatGroups[i], false, Factorial(dataMat[i, 1])/2);
>     RECOG.TestGroup(altMatGroups[i], true, Factorial(dataMat[i, 1])/2);
>     RECOG.TestGroup(symMatGroups[i], false, Factorial(dataMat[i, 1]));
> od;
...
gap> g1 := gens[1];;
gap> img1 := RECOG.FindImageSn(ri, 11, g1, isoData.stdGens[1], isoData.stdGens[2],
>                        isoData.sieve[1], isoData.sieve[2]);;
gap> CycleStructurePerm(img1);
[ ,,,,,,,,, 1 ]
gap> g2 := gens[2];;
gap> img2 := RECOG.FindImageSn(ri, 11, g2, isoData.stdGens[1], isoData.stdGens[2],
>                        isoData.sieve[1], isoData.sieve[2]);;
