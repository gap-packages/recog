...
local stringOut, dir, find, streamIn, streamOut, files, file, i;
    ...
    Process(dir, find, streamIn, streamOut, [".", "-path", "./.git/*", "-prune", "-o", "!", "-type", "d", "-regex", ".*.\\(xml\\|autodoc\\|gd\\|gi\\|g\\|tst\\)", "-print"]);
    CloseStream(streamIn);
    CloseStream(streamOut);
    ...
    for file in files do
        for i in [1..Length(csv)] do
            ...
            if csv[i].TYPE = "Attr" then
                Exec(JoinStringsWithSeparator(["sed -i \"s/\\bHas",csv[i].OLD_NAME,"\\b/Has",csv[i].NEW_NAME,"/g\" ",file], ""));
                Exec(JoinStringsWithSeparator(["sed -i \"s/\\bSet",csv[i].OLD_NAME,"\\b/Set",csv[i].NEW_NAME,"/g\" ",file], ""));
            fi;
        od;
    od;
end;
...
GenerateRenamingTableAndIndex := function()
local tablefile, csv, indexfile, i;

    tablefile := "./doc/renaming_table.xml";
    indexfile := "./doc/renaming_index.xml";

    PrintTo(tablefile);
    PrintTo(indexfile);

    AppendTo(tablefile, "<Table Align=\"|l|l|\">\n");
    AppendTo(tablefile, "<Caption>Renaming</Caption>\n");
    AppendTo(tablefile, "<HorLine/>\n");

    AppendTo(tablefile, "<Row>\n");
    AppendTo(tablefile, "\t<Item> <E>","Old Name","</E> </Item>\n");
    AppendTo(tablefile, "\t<Item> <E>","New Name","</E> </Item>\n");
    AppendTo(tablefile, "</Row>\n");
    AppendTo(tablefile, "<HorLine/>\n");
    ...
    for i in [1..Length(csv)] do
        AppendTo(tablefile, "<Row>\n");
        AppendTo(tablefile, "\t<Item> <C>",csv[i].OLD_NAME,"</C> </Item>\n");
        AppendTo(tablefile, "\t<Item> <Ref ",csv[i].TYPE,"=\"", csv[i].NEW_NAME, "\"/> </Item>\n");
        AppendTo(tablefile, "</Row>\n");
        AppendTo(tablefile, "<HorLine/>\n");

        AppendTo(indexfile, "<Index Key=\"",csv[i].OLD_NAME,"\"><C>",csv[i].OLD_NAME,"</C></Index>\n");
    od;

    AppendTo(tablefile, "</Table>\n");
end;
